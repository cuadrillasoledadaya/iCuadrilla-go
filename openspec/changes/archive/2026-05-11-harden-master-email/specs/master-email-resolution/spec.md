# Delta for Master Email Resolution
...
### Requirement: Client-Side Master Email Fallback

(Reason: Security vulnerability — any user knowing the hardcoded email could spoof superadmin role by bypassing server-side check. Server action `checkIsMaster()` is the sole authority.)

### Requirement: Hardcoded Default Email in Server Actions/Middleware

(Reason: Defeats the purpose of environment-based configuration. If `MASTER_EMAIL` is not configured, the system must fail closed, not silently fall back to a known email.)
