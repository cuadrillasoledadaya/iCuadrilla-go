...
export default function RecuperarPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { data: isAuthorized, error: rpcError } = await supabase.rpc('es_email_autorizado', {
    ...
    const { data: isRegistered, error: regError } = await supabase.rpc('es_email_registrado', {
      email_input: cleanEmail,
    });

    if (regError || !isRegistered) {
      setError(
        'Este email está autorizado pero aún NO has creado tu cuenta. Por favor, regístrate primero.'
      );
      setLoading(false);
      return;
    }
        ...
        <div className="w-full max-w-sm space-y-8 text-center">
          ...
          <div>
            ...
            <p className="mt-2 text-xs text-neutral-500">
              Revisa tu bandeja de entrada (y spam) y haz clic en el enlace para establecer una
              nueva contraseña.
            </p>
          </div>
          <Link
