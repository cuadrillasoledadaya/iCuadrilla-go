...
export default function RolesPage() {
  ...
  // Filter: exclude master account (resolved via server env, not hardcoded) + search query
  const filteredCostaleros = costaleros.filter((c) => {
    const matchesSearch = `${c.nombre} ${c.apellidos}`
      .toLowerCase()
      .includes(searchQuery.toLowerCase());
    // When current user is master, exclude their own account from the list
    const isMasterSelf =
      isMaster && userEmail && c.email?.toLowerCase() === userEmail.toLowerCase();
    return matchesSearch && !isMasterSelf;
  });
