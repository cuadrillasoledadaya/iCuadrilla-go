...
export default function GestionRelevos() {
  ...
  const fetchInitialData = async () => {
    ...
    try {
      // 1. Mudas
      let { data: mudasData } = await supabase
        .from('muda_nombres')
        .select('*')
  ...
  const getPosicionLabel = (t: number, p: number) => {
    ...
    switch (p) {
      case 1:
        return isFirstOrLast ? 'Patero Izq' : 'Costero Izq';
      case 2:
        return isFirstOrLast ? 'Patero Der' : 'Costero Der';
      case 3:
        return 'Fijador Izq';
      case 4:
        return 'Fijador Der';
    ...
    <div className="min-h-screen pb-32 bg-background animate-in fade-in duration-700">
          ...
          <div className="flex justify-between items-end">
            <div className="space-y-1">
              ...
              <p className="text-[10px] text-neutral-900 font-black uppercase tracking-[0.2em]">
                {mudas.find((m) => m.id === activeMudaId)?.nombre || 'RELEVO'}
              </p>
            </div>
        ...
        {Array.from({ length: TRABAJADERAS_COUNT }, (_, i) => i + 1).map((t) => (
          <div key={t} className="space-y-6">
            <div className="flex items-center gap-3 px-2">
              ...
              <h2 className="text-xl font-black text-neutral-900 uppercase tracking-tighter italic">
                TRABAJADERA {t}
              </h2>
              <div className="h-px flex-1 bg-gradient-to-r from-transparent via-neutral-200 to-transparent" />
            </div>
            ...
            <div className="grid grid-cols-2 gap-4">
              {[1, 2, 3, 4].map((p) => {
                ...
                const isOutOfPosition =
                  costalero && !posLabel.toLowerCase().includes(costalero.puesto.toLowerCase());
                    ...
                    <div className="flex justify-between items-start">
                      ...
                      <div className="flex flex-col items-end gap-1">
                        ...
                        {(releveData?.suplemento != null ||
                          (!isOutOfTrabajadera && costalero?.suplemento != null)) && (
              ...
              <div className="col-span-2 flex justify-center pt-2">
                {(() => {
                  ...
                  const isCorrienteOutOfPosition =
                    costaleroCorriente && costaleroCorriente.puesto !== 'Corriente';
                  const isCorrienteOutOfTrabajadera =
                    costaleroCorriente && Number(costaleroCorriente.trabajadera) !== Number(t);
                  const isCorrienteOutOfPlace =
                    isCorrienteOutOfPosition || isCorrienteOutOfTrabajadera;
                  ...
                  const getCorrienteAsistenciaBg = () => {
                    if (!costaleroCorriente)
                      return 'bg-neutral-50/50 border-dashed border-neutral-200';
                    if (selectedPos?.t === t && selectedPos?.p === 5)
                      return 'border-primary bg-primary/5 shadow-lg scale-105 z-10';
                    ...
                    >
                      <div className="flex justify-center items-center gap-2">
                        ...
                        <div className="flex flex-col items-start gap-1">
                          ...
                          {(releveData?.suplemento != null ||
                            (!isCorrienteOutOfTrabajadera &&
                              costaleroCorriente?.suplemento != null)) && (
                            <span className="text-[8px] font-black bg-primary/10 text-primary px-1.5 py-0.5 rounded-full">
          ...
          >
            <div className="text-center space-y-2">
              ...
              <p className="text-[10px] font-black text-neutral-900 uppercase tracking-widest">
                Trabajadera {selectedPos?.t} •{' '}
                {selectedPos && getPosicionLabel(selectedPos.t, selectedPos.p)}
              </p>
            </div>
                ...
                <div className="flex-[2] flex flex-col justify-end">
                  ...
                  >
                    ...
                    {searchOther ? (
                      ...
                      <div className="w-4 h-4 border-2 border-neutral-900 rounded" />
                    )}
                  </button>
                </div>
              </div>
      ...
      {showMudaModal && (
        <div
          ...
          onClick={() => setShowMudaModal(false)}
        >
          <div
            className="w-full max-w-sm bg-white rounded-[32px] p-8 space-y-6 shadow-2xl animate-in zoom-in duration-200"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="text-center space-y-2">
            ...
            <div className="space-y-4">
              ...
              />
              <Button
                className="w-full h-14 bg-primary hover:bg-primary/90 rounded-2xl font-black uppercase tracking-widest"
                onClick={handleManageMuda}
              >
                Guardar Cambios
              </Button>
