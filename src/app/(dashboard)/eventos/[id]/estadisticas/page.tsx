...
function EstadisticasContent() {
  ...
  const fetchData = async () => {
    ...
    if (eventData) {
      ...
      const [costalerosRes, asistenciasRes] = await Promise.all([
        supabase
          .from('costaleros')
          .select('id, nombre, apellidos, apodo, puesto, trabajadera')
          .eq('rol', 'costalero'),
      ...
      costaleros.forEach((c) => {
        ...
        if (t >= 1 && t <= 7) {
          ...
          else if (asis?.estado === 'justificado' || asis?.estado === 'justificada')
            tStats[idx].justificados++;
  ...
  const progress =
    stats.total > 0 ? Math.round(((stats.presentes + stats.justificados) / stats.total) * 100) : 0;
  ...
  const getStatusColor = (estado?: string) => {
    switch (estado) {
      case 'presente':
        return 'bg-emerald-500 text-neutral-900';
      case 'justificado':
      case 'justificada':
      ...
      <div className="p-8 rounded-[40px] bg-white border border-black/5 shadow-xl space-y-6 relative overflow-hidden">
        ...
        <div className="grid grid-cols-4 gap-2 pt-2 relative z-10">
          {[
            ...
            <div
              key={idx}
              className="bg-neutral-50 p-3 rounded-2xl border border-black/5 text-center"
            >
              <span className={cn('text-lg font-black italic block', s.color)}>{s.val}</span>
              <span className="text-[7px] font-black text-neutral-400 tracking-widest">
                {s.label}
              </span>
            </div>
          ))}
                ...
                <div className="flex-1 space-y-2">
                  ...
                  <div className="flex gap-1.5 h-3.5">
                    {Array.from({ length: t.presentes }).map((_, i) => (
                      <div
                        key={`p-${i}`}
                        className="flex-1 bg-emerald-500 rounded-full shadow-[0_0_8px_rgba(16,185,129,0.3)]"
                      />
                    ))}
                    {Array.from({ length: t.justificados }).map((_, i) => (
                      <div
                        key={`j-${i}`}
                        className="flex-1 bg-amber-400 rounded-full shadow-[0_0_8px_rgba(251,191,36,0.2)]"
                      />
                    ))}
                    {Array.from({ length: t.ausentes }).map((_, i) => (
                      <div
                        key={`a-${i}`}
                        className="flex-1 bg-red-400 rounded-full shadow-[0_0_8px_rgba(248,113,113,0.2)]"
                      />
                    ))}
                    {Array.from({ length: t.pendientes }).map((_, i) => (
                      <div
                        key={`n-${i}`}
                        className="flex-1 bg-neutral-100 border border-neutral-200/50 rounded-full"
                      />
                    ))}
                  </div>
                </div>
                ...
                <div className="flex gap-3 overflow-x-auto pb-4 px-2 no-scrollbar animate-in slide-in-from-top-2 duration-300">
                  {t.costaleros.map((c) => (
                    ...
                    >
                      ...
                      <span className="text-[9px] font-bold opacity-80 tracking-tighter uppercase">
                        {c.puesto || 'Costalero'}
                      </span>
                    </div>
                  ))}
