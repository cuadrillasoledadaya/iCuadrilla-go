    ...
    <div className="min-h-screen bg-[#f8fafc] pb-32 animate-in fade-in duration-700">
      ...
      <header className="relative p-6 flex items-center justify-center bg-[#f8fafc]">
        ...
        <div className="text-center space-y-1">
          <h1 className="text-lg font-black tracking-tight uppercase text-neutral-900 tracking-[0.2em]">
            Tramitar Baja
          </h1>
        </div>
      </header>
      ...
      <main className="px-6 space-y-8">
        <div className="bg-red-50 p-6 rounded-[32px] border border-red-100 space-y-4 shadow-sm">
          ...
          <p className="text-xs text-red-500/80 font-bold leading-relaxed">
            Estás a punto de tramitar la baja de{' '}
            <span className="font-black underline text-neutral-900">
              {costalero?.apodo
                ? `${costalero.apodo} (${costalero.nombre})`
                : `${costalero?.nombre} ${costalero?.apellidos}`}
            </span>
            . Esta acción lo retirará del listado activo de la cuadrilla.
          </p>
        </div>
