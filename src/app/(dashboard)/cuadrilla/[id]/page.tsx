...
export default function FichaCostalero() {
  ...
  useEffect(() => {
    const fetchCostalero = async () => {
      const { data } = await supabase.from('costaleros').select('*').eq('id', params.id).single();
    ...
    return (
      <div className="p-6 text-center space-y-4 min-h-screen bg-neutral-950 flex flex-col items-center justify-center">
        <p className="text-neutral-500 font-bold uppercase tracking-widest">
          No se ha encontrado el costalero.
        </p>
        <Button onClick={() => router.back()} className="bg-primary text-white font-bold">
          Volver
        </Button>
      </div>
    );
    ...
    <div className="min-h-screen bg-neutral-950 pb-32 animate-in fade-in duration-700">
      ...
      <header className="relative p-6 flex items-center justify-center">
        ...
        <div className="text-center space-y-1">
          <h1 className="text-lg font-black tracking-tight uppercase text-white tracking-[0.2em] text-shadow-sm">
            Ficha del Hermano
          </h1>
        </div>
        <button
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-neutral-900/50 p-5 rounded-[24px] border border-white/5 flex flex-col items-center justify-center gap-3 hover:bg-neutral-900/80 transition-colors group">
            ...
            <div className="text-center">
              ...
              <p className="text-xl font-black text-white">
                {costalero.altura} <span className="text-xs text-neutral-600 font-bold">m</span>
              </p>
            </div>
          </div>
          ...
          <div className="bg-neutral-900/50 p-5 rounded-[24px] border border-white/5 flex flex-col items-center justify-center gap-3 hover:bg-neutral-900/80 transition-colors group">
            ...
            <div className="text-center">
              ...
              <p className="text-xl font-black text-white">
                {costalero.suplemento ? `${costalero.suplemento}` : '--'}{' '}
                <span className="text-xs text-neutral-600 font-bold">cm</span>
              </p>
            </div>
          </div>
          ...
          <div
            className="bg-neutral-900/50 p-5 rounded-[24px] border border-white/5 flex flex-col items-center justify-center gap-3 hover:bg-neutral-900/80 transition-colors group cursor-pointer"
            onClick={() => window.open(`mailto:${costalero.email}`)}
          >
            <div className="p-2.5 bg-neutral-800/50 rounded-xl text-neutral-400 group-hover:text-primary transition-colors">
              <Mail size={20} />
            </div>
          ...
          <div className="col-span-2 grid grid-cols-2 gap-4">
            <div
              ...
              onClick={() =>
                costalero.telefono && (window.location.href = `tel:${costalero.telefono}`)
              }
            >
              <Phone
                size={24}
            ...
            <div
              ...
              onClick={() =>
                costalero.telefono &&
                window.open(`https://wa.me/34${costalero.telefono.replace(/\s+/g, '')}`, '_blank')
              }
            >
              <MessageCircle
                size={24}
