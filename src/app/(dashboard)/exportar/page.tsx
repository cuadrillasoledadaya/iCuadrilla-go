...
export default function ExportarDatos() {
      ...
      if (temporadas?.id) {
        ...
        if (eventos && costalerosData) {
          const statsPromises = eventos.map(async (evento) => {
            ...
            const detailedAsistencias = (asistencias || [])
              .map((a) => {
              ...
              .sort(
                (a, b) => a.trabajadera - b.trabajadera || a.apellidos.localeCompare(b.apellidos)
              );
            ...
            const justificados = detailedAsistencias.filter(
              (a) => a.estado === 'justificado' || a.estado === 'justificada'
            ).length;
  ...
  const exportCostalerosPDF = async () => {
    ...
    doc.text(
      `Temporada ${temporadaActiva || '-'} - Generado: ${new Date().toLocaleDateString('es-ES')}`,
      14,
      28
    );
  ...
  const generateEstadisticasPDF = async (eventsToExport: EventoStats[], filename: string) => {
    ...
    eventsToExport.forEach((evento, index) => {
      ...
      doc.setFontSize(9);
      doc.text(
        `Estado: ${evento.estado} | Fecha: ${new Date(evento.fecha_inicio).toLocaleDateString('es-ES')} | Hora: ${new Date(evento.fecha_inicio).toLocaleTimeString('es-ES', { hour: '2-digit', minute: '2-digit' })}`,
        18,
        yPos + 18
      );
      doc.text(
        `Pres: ${evento.presentes} | Aus: ${evento.ausentes} | Just: ${evento.justificados}`,
        18,
        yPos + 24
      );
  ...
  const exportQRCodesPDF = async () => {
    ...
    // Sort trabajaderas
    const sortedTrabajaderas = Object.keys(trabajaderasGroups)
      .map(Number)
      .sort((a, b) => a - b);
    ...
    for (const trabajaderaNum of sortedTrabajaderas) {
      ...
      for (const costalero of costalerosList) {
        ...
        try {
          ...
          doc.text(fullName, xPos + qrSize / 2, yPos + qrSize + 5, {
            align: 'center',
            maxWidth: qrSize,
          });
          ...
          doc.setFontSize(8);
          doc.setTextColor(100);
          doc.text(costalero.puesto, xPos + qrSize / 2, yPos + qrSize + 10, {
            align: 'center',
            maxWidth: qrSize,
          });
          doc.setTextColor(0);
    ...
    <div className="p-6 space-y-8 pb-32 animate-in fade-in duration-700 bg-background min-h-screen">
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="text-lg font-black text-neutral-900 uppercase tracking-tight">
              Listado de Cuadrilla
            </h2>
            <p className="text-xs text-neutral-400 font-medium">
              {costaleros.length} costaleros registrados
            </p>
          </div>
        </div>
        ...
        >
          ...
          <div className="text-left">
            <p className="text-[10px] uppercase tracking-[0.2em] text-emerald-100/80 leading-none mb-1 font-bold">
              Listado Cuadrilla
            </p>
            <p className="text-sm uppercase tracking-tight text-white italic">
              Extendido (Con Hojas en Blanco)
            </p>
          </div>
        </Button>
      </section>
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-neutral-400 font-medium">
              Exportar todos los QR por trabajaderas
            </p>
          </div>
        </div>
        <Button
      ...
      <section className="space-y-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-neutral-400 font-medium">
              {eventosStats.length} eventos (Reporte Global)
            </p>
          </div>
        </div>
        <div className="grid grid-cols-2 gap-3">
      ...
      <section className="space-y-4 pt-4 border-t border-black/5">
        <div className="space-y-2">
          <h2 className="text-sm font-black text-neutral-900 uppercase tracking-tight ml-1">
            Exportar Por Evento
          </h2>
          <select
      ...
      <section className="bg-white p-5 rounded-[24px] border border-black/5 shadow-sm space-y-4">
        <h3 className="text-xs font-black uppercase text-neutral-400 tracking-widest">
          Vista Rápida
        </h3>
        <div className="grid grid-cols-2 gap-3">
